...
Item {
  ...
  Settings {
    id: appearance
    overrides: root.settingsEntry(root.shell ? root.shell.shellConfig : {})
  }

  function settingsEntry(config) {
    var id = root.manifest ? root.manifest.id : "stash.menu"
    var layout = config && config.bar ? config.bar.layout || {} : {}
    var entries = (layout.left || []).concat(layout.center || [], layout.right || [], config.plugins || [])
    for (var i = 0; i < entries.length; i++) if (entries[i].id === id) return entries[i]
    return {}
  }

  function editSettings(patch, reset) {
    if (!root.shell) return JSON.stringify({ok: false, error: "Shell is not ready"})
    var error = appearance.validate(patch)
    if (error) return JSON.stringify({ok: false, error: error})
    var found = false
    root.shell.mutateShellConfig(function(config) {
      var entry = root.settingsEntry(config)
      if (!entry.id) return
      found = true
      if (reset) for (var key in appearance.defaults) delete entry[key]
      else for (var key in patch) entry[key] = patch[key]
    })
    return JSON.stringify(found ? {ok: true, settings: appearance.snapshot()} : {ok: false, error: "Plugin entry missing"})
  }

  SettingsWindow {
    id: settingsWindow
    settingsModel: appearance
    onUpdateRequested: function(patch) { report(root.editSettings(patch, false)) }
    onResetRequested: report(root.editSettings({}, true))
    onPreviewRequested: root.openRoute("root")
  }
  ...
  IpcHandler {
    ...
    function get(): string { return JSON.stringify(appearance.snapshot()) }
    function set(json: string): string {
      var patch
      try { patch = JSON.parse(json) } catch (e) { return JSON.stringify({ok: false, error: "Invalid JSON"}) }
      return root.editSettings(patch, false)
    }
    function reset(): string { return root.editSettings({}, true) }
    function open(): void { root.openRoute("root") }
    function apps(): void { root.openRoute("apps") }
    function close(): void { root.close() }
    function toggle(): void { if (root.opened) root.close(); else root.openRoute("root") }
  }


  property string fontFamily: appearance.value("fontFamily")
  ...
  property color background: appearance.backgroundColor()
  property color foreground: appearance.foregroundColorValue()
  property color border: appearance.borderColorValue()
  property var borderSpec: Border.flat(border, 1)
  property color scrim: "transparent"
  property color selectedBackground: appearance.selectionBackgroundColor()
  property color hoverBackground: appearance.selectionBackgroundColor()
  property color selectedText: appearance.selectionTextColorValue()
  property color selectedBorder: appearance.selectionBorderColorValue()
  property var selectedBorderSpec: Border.flat(selectedBorder, 1)
  readonly property real rowReservedBorderLeft: Border.left(selectedBorderSpec)
  readonly property real rowReservedBorderRight: Border.right(selectedBorderSpec)
  readonly property int cornerRadius: appearance.value("radius")
  property int contentMargin: appearance.value("padding")
  property int contentTopMargin: 2
  property int headerHeight: appearance.value("headerHeight")
  property int contentSpacing: 6
  property int baseRowHeight: appearance.value("rowHeight")
  property int detailRowHeight: appearance.value("detailRowHeight")
  ...
  property int rowSpacing: appearance.value("rowSpacing")
  property int dividerHeight: Style.space(17)
  property bool searchDivider: false
  property int layoutSerial: 0
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : appearance.value("width"), panel.width - Style.gapsOut * 2)
  property bool resultsVisible: root.dmenuActive || appearance.value("resultBehavior") === "expanded" || !!root.filterText
  property int searchHeadingHeight: 0
  onResultsVisibleChanged: {
    if (resultsVisible && panel.visible) {
      panel.cardTop = -1
      panel.maxRowsHeight = -1
    }
  }
  property int visibleRowsHeight: root.resultsVisible
    ? root.searchHeadingHeight + (root.dmenuActive ? dmenuRowListHeight(layoutSerial, displayModel.count, filterText) : rowListHeight(layoutSerial, displayModel.count, filterText, searchDivider))
    : 0
  property int cardHeight: root.dmenuActive
    ? Math.min(contentTopMargin + contentMargin + headerHeight + (mode === "input" || !resultsVisible ? 0 : contentSpacing + visibleRowsHeight), panel.height - Style.gapsOut * 2)
    : Math.min(contentTopMargin + contentMargin + headerHeight + (!resultsVisible ? 0 : contentSpacing + visibleRowsHeight), panel.height - Style.gapsOut * 2)
  ...
  function availableRowsHeight() {
    ...
    var available = panel.height - top - Style.gapsOut - root.contentTopMargin - root.contentMargin - root.headerHeight - root.contentSpacing
    ...
    return Math.min(available, appearance.value("maxResultsHeight"))
  }
  ...
  function rowListHeight(_serial, _count, _filter, _divider) {
    if (displayModel.count === 0) return 96
  ...
  PanelWindow {
    ...
    // Keep the card centered as results change. Preserve the initial row
    // height limit so searching does not expand the results beyond the fold.
    ...
    readonly property int effectiveCardTop: centeredTop
    function freezeCardTop() {
      if (visible && root.resultsVisible && cardTop < 0) {
    ...
    Repeater {
      ...
      Rectangle {
        ...
        y: panel.effectiveCardTop + 7 - (index + 1) * 3
        width: root.cardWidth + (index + 1) * 6
        height: Math.min(root.cardHeight, panel.height - Style.gapsOut - panel.effectiveCardTop) + (index + 1) * 6
        radius: root.cornerRadius + (index + 1) * 3
        color: "#08000000"
      }
    }
    ...
    BorderSurface {
      ...
      topPadding: root.contentTopMargin
      rightPadding: root.contentMargin
      ...
      Column {
        ...
        Rectangle {
          ...
          // Draw the search symbol natively so no icon font is required.
          Item {
            id: searchIcon
            width: appearance.value("searchIconSize"); height: width
            ...
            Rectangle {
              x: Math.round(parent.width * 0.61)
              y: Math.round(parent.height * 0.60)
          ...
          Text {
            ...
            anchors.verticalCenter: parent.verticalCenter
            text: root.filterText || (root.dmenuActive ? (root.dmenuPrompt + "…") : appearance.value("placeholder"))
            color: root.foreground
            opacity: root.filterText ? 1 : 0.58
            font.family: root.fontFamily
            font.pixelSize: appearance.value("searchFontSize")
        ...
        Item {
          ...
          ListView {
            id: resultList
            anchors.fill: parent
            anchors.topMargin: root.searchHeadingHeight
            model: displayModel
            clip: true
            ...
            delegate: BorderSurface {
              ...
              readonly property bool hasIcon: appearance.value("showIcons")

              width: ListView.view.width - appearance.value("resultSideInset") * 2
              x: appearance.value("resultSideInset")
              height: root.rowHeightForDetail(row.detail)
              radius: appearance.value("selectionRadius")
              color: row.hasCursor
                ? root.selectedBackground
                : (root.cursorActive ? "transparent" : (row.isHovered ? root.hoverBackground : "transparent"))
              ...
              Item {
                id: iconText
                visible: row.hasIcon
                width: appearance.value("iconSize"); height: width
                anchors.left: parent.left
                anchors.leftMargin: appearance.value("resultInnerPadding")
                anchors.verticalCenter: parent.verticalCenter
                ...
                Text {
                  ...
                  text: row.icon
                  color: root.foreground
                  font.family: row.iconFont.length > 0 ? row.iconFont : "Iosevka Nerd Font"
                  font.pixelSize: Math.round(appearance.value("iconSize") * 0.72)
                }
                Image {
                  anchors.centerIn: parent
                  width: appearance.value("iconSize"); height: width
              ...
              Column {
                ...
                anchors.leftMargin: row.hasIcon ? 10 : appearance.value("resultInnerPadding")
                anchors.right: appDetail.visible ? appDetail.left : trail.left
                ...
                Text {
                  ...
                  width: parent.width
                  text: row.label
                  color: (row.hasCursor || row.isHovered) ? root.selectedText : root.foreground
                  font.family: root.fontFamily
                  font.pixelSize: appearance.value("resultFontSize")
                ...
                Text {
                  ...
                  opacity: 0.52
                  font.family: root.fontFamily
                  font.pixelSize: appearance.value("detailFontSize")
                  elide: Text.ElideRight
                }
              }
              ...
              Text {
                ...
                anchors.rightMargin: Style.space(6)
                anchors.verticalCenter: parent.verticalCenter
                width: row.isApp && !root.filterText && row.detail.length > 0 ? Math.min(220, parent.width * 0.32) : 0
                visible: row.isApp && !root.filterText && row.detail.length > 0
                textFormat: Text.PlainText
                text: row.detail
                color: row.hasCursor ? root.selectedText : root.foreground
                opacity: row.hasCursor ? 0.78 : 0.52
                font.family: root.fontFamily
                font.pixelSize: appearance.value("detailFontSize")
              ...
              Row {
                ...
                anchors.right: parent.right
                anchors.rightMargin: root.rowReservedBorderRight + appearance.value("resultInnerPadding")
